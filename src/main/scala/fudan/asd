...
class JdbcSourceV2 extends DataSourceV2 with ReadSupport with WriteSupport {
  ...
  override def createWriter(
                             ...
                             schema: StructType,
                             mode: SaveMode,
                             options: DataSourceOptions): Optional[DataSourceWriter] = Optional.of(new JdbcDataSourceWriter(
    options.get("url").get(),
    options.get("user").get(),
    options.get("password").get(),
    options.get("table").get(),
...
class JdbcDataReader(
  ...
  def next() = {
    if (rs == null) {
      ...
      if (wheres.nonEmpty) sqlBuilder ++= " AND " + wheres.mkString(" AND ")
...
class JdbcDataSourceWriter(
  ...
  override def createWriterFactory() = new JdbcDataWriterFactory(url, user, password, table, schema)
...
class JdbcDataWriterFactory(
  ...
  override def createDataWriter(
                                 partitionId: Int,
                                 attemptNumber: Int) = new JdbcDataWriter(url, user, password, table, schema)
}
