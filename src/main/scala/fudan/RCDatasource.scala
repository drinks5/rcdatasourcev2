...
class CsvBatchTable(path: String) extends Table with SupportsRead {
  ...
  override def newScanBuilder(options: CaseInsensitiveStringMap) = new CsvScanBuilder(path)
}
...
class CsvScanBuilder(path: String) extends ScanBuilder {
  override def build() = new CsvScan(path)
}
...
class CsvScan(path: String) extends Scan with Batch {
  ...
  override def createReaderFactory() = new CsvPartitionReaderFactory()
}
